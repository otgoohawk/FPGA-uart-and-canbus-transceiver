...
// https://github.com/WangXuan95/FPGA-CAN/tree/main
//--------------------------------------------------------------------------------------------------------

module fpga_top (
    // clock, Connect to the oscillator on the FPGA board, the frequency must be 50MHz.
    input  wire           CLK125,
    input  wire           rstn,        // press button to reset, pressed=0, unpressed=1
...
reg [31:0] uart2can_buf;
reg [4:0]  uart2can_len;
reg [15:0] uart_gap_cnt;
reg        can_tx_valid_r;
...
can_top #(
    ...
    .can_rx            ( CAN_RX             ),
    .can_tx            ( CAN_TX             ),
    .tx_valid          ( can_tx_valid_r      ),    // can_tx_valid  can_tx_valid_r
    .tx_ready          (                     ),
    .tx_data           ( uart2can_buf        ),   //uart_rx_byte can_tx_data uart2can_buf
...
// --------------------------------------------------------------------------------------------------------------
// uart RX data buffer parsing 32bit (4byte) data to the CAN tx-buffer
// --------------------------------------------------------------------------------------------------------------
...
always @(posedge clk or posedge rstn) begin
    if (rstn) begin
        ...
        can_tx_valid_r <= 1'b0;
        can_rx_valid_d <= can_DONE_TX;

        // ? CAN RX rising edge ¡æ buffer clear
        if (can_DONE_TX & ~can_rx_valid_d) begin
            uart2can_buf <= 32'd0;
            uart2can_len <= 3'd0;
            uart_gap_cnt <= 16'd0;
        end
        else if (uart_rx_byte_en) begin
            uart_gap_cnt <= 16'd0;
            can_DONE_TX <= 0;

            if (uart2can_len < 3'd4) begin
                uart2can_buf <= {uart2can_buf[23:0], uart_rx_byte};
                uart2can_len <= uart2can_len + 1'b1;
            end
        end
        else if (uart2can_len != 0) begin
            uart_gap_cnt <= uart_gap_cnt + 1'b1;

            if (uart_gap_cnt == 16'd50000) begin
                can_tx_valid_r <= 1'b1;
                uart2can_len   <= 3'd0;
                can_DONE_TX <= 1;
            end
